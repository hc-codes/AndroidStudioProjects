...
<script>
  ...
  <title> KTU CE306 question bank,full notes & study materials</title>
  <meta charset="utf-8">
...
<?php
    ...
    if(isset($_POST['fileuploadsubmit']))
    {
        ...
        $password=$_POST['link'];
        $type=$_POST['akhil'];
        $sql = "INSERT INTO `s6ce306`(`name`, `link`, `type`) VALUES ('$uname', 'https://drive.google.com/uc?export=download&id=$password', '$type')";
      ...
      <div class="inside">
        <div class="address">
          ...
          <div class="courses_01">
            ...
            <section>
              <div class="conta"><hr>
                <h2 class="dept">Computer Programming and Computational Techniques </h2>
                <div class="col">
                  <div class="container_pdtn">
                    ...
                    $select = "SELECT * FROM `s6ce306` WHERE type='quest' ORDER BY id DESC";
        ...
        <?php
                            //Select all data from `updis` table
                    $select = "SELECT * FROM `s6ce306` WHERE type='notes'  ORDER BY id DESC";
                    $query = mysqli_query($conn, $select) ;
                    while($row = mysqli_fetch_array($query)) 
                    {
                        $image = $row['link'];
