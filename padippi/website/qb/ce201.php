...
<script>
  ...
  <title> KTU CE201 question bank,previous year question paper, solved questions</title>
  <meta charset="utf-8">
...
<?php
    ...
    if(isset($_POST['fileuploadsubmit']))
    {
        ...
        $type=$_GET['p2'];
        $name=$_GET['p1'];
        $sql = "INSERT INTO `s3ce201`(`name`, `link`, `type`, `Author`) VALUES ('$uname', '$password', '$type', '$name')";
...
<br>  
      <div class="inside">
              ...
              <div class="conta"><hr>
                ...
                <div class="col">
                  <div class="container_pdtn">
                    ...
                    $select = "SELECT * FROM `s3ce201` WHERE type='quest' ORDER BY id DESC";
        ...
        <?php
                            //Select all data from `updis` table
                    $select = "SELECT * FROM `s3ce201` WHERE type='module'  ORDER BY id DESC";
                    $query = mysqli_query($conn, $select) ;
                    while($row = mysqli_fetch_array($query)) 
                    {
                        $image = $row['link'];
