...
<script>
  ...
  <title> KTU CS201 Discrete Computational Structures question bank,previous year question paper, solved questions</title>
  <meta charset="utf-8">
  ...
  <meta name="keywords" content="cse, ktu cse, ktu cse full notes, questuion banks, previous year question paper solved, MA201 Notes, CS201 Notes, CS204 Notes, CS206 Notes, CS208 Notes, HS200 Notes,HS210 Notes, Full notes of all cse subjects">
...
<?php
    ...
    if(isset($_POST['fileuploadsubmit']))
    {
        ...
        $type=$_GET['p2'];
        $name=$_GET['p1'];
        $sql = "INSERT INTO `s3cs201`(`name`, `link`, `type`, `Author`) VALUES ('$uname', '$password', '$type', '$name')";
...
<br>  
      <div class="inside">
        <div class="address">
          ...
          <div class="courses_01">
            ...
            <section>
              <div class="conta"><hr>
                <h2 class="dept">Discrete Computational Structures </h2>
                <div class="col">
                  <div class="container_pdtn">
                    ...
                    $select = "SELECT * FROM `s3cs201` WHERE type='quest' ORDER BY id DESC";
        ...
        ?>
          <div class="student_notes">
            <h2 class="tt" style="padding-top: 20px;">Module wise question bank</h2>
        </div>
        <?php
                            //Select all data from `updis` table
                    $select = "SELECT * FROM `s3cs201` WHERE type='module'  ORDER BY id DESC";
                    $query = mysqli_query($conn, $select) ;
                    while($row = mysqli_fetch_array($query)) 
                    {
                        $image = $row['link'];
    ...
    <?php
                        //Select all data from `updis` table
               /* $select = "SELECT * FROM `s3pdtn` WHERE type='lectur'";
                $query = mysqli_query($conn, $select) ;
                while($row = mysqli_fetch_array($query)) 
                {
                    $image = $row['link'];
